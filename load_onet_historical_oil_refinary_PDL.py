...
df = pd.read_csv(r"Dados ICP e Bruker Refinaria PRLE_Limpa_Preparada.csv", delimiter=';', low_memory=False)
df = pd.DataFrame(df)
...
result_df['timestamp'] = pd.to_datetime(result_df['timestamp'], format='%d/%m/%Y %H:%M:%S')
...
result_df = result_df.drop_duplicates(subset=['timestamp', 'measurement_desc'])

# Please, the file out needs to be uploaded in onet_tag_values_historical_tmp/Primavera_do_Leste hdfs path

result_df.to_csv("oil_refinary_pdl.csv", sep='\t', index=0)